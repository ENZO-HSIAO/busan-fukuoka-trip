...
function renderFlights() {
  const filtered = state.flights
    .filter((f) => {
      ...
      // "me" / "partner" filters also include shared flights, since a
      // shared flight belongs to both people.
...
function renderItinerary() {
  ...
  // Always show the preset trip dates as tabs, plus any extra dates that
  // already have items outside that range (e.g. leftover seed data),
  // merged and sorted chronologically.
  const dates = Array.from(new Set([...ITIN_DATES, ...Object.keys(grouped)])).sort();
    ...
    ${dates.length === 0
      ...
      : `<div style="margin-bottom:22px">
          <div style="font-size:13px;color:var(--navy);letter-spacing:1px;margin-bottom:14px;font-weight:600">
            ${fmtDate(activeDate)} ${(ITIN_CITY_BY_DATE[activeDate] || dayList[0]?.city) ? `· ${esc(ITIN_CITY_BY_DATE[activeDate] || dayList[0].city)}` : ""}
